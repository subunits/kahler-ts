...
--   3. Walk epsilon steps in order, consuming edges incrementally from the
--      sorted list. State (edges, membership array, union-find, triangle count)
--      is carried forward, never rebuilt from scratch.
--   4. Triangle count: when a new edge (i,j) is added, count k s.t. both
--      (i,k) and (j,k) are already present — O(n) per new edge.
--
-- Cost: O(n² log n + n² × n) = O(n³) worst case but with a much smaller
-- constant, and critically the n³ work is spread across the whole filtration
-- rather than repeated at every epsilon step.
...
vrFiltration pts nSteps =
  let n = length pts
      ...
      -- Incremental walk: carry state forward across epsilon steps
      -- State: (remaining sorted edges, current edges, edge array, UF, tri count)
      ...
      step (remaining, curEdges, eArr, uf, triCount) eps =
        let -- Pull all edges with weight <= eps from the front of the sorted list
            (batch, rest) = span (\(w,_,_) -> w <= eps) remaining

            -- For each new edge (i,j), count triangles it closes:
            -- a triangle is closed when both (i,k) and (j,k) already exist
            newTrisFor i j =
              length [ () | k <- [0..n-1]
                          , k /= i, k /= j
            ...
            -- Add edges one at a time, updating state incrementally
            addOne (es, earr, u, tc) (_, i, j) =
              let tc'   = tc + newTrisFor i j
                  earr' = addEdge earr i j
                  u'    = unionUF u i j
                  es'   = (i,j) : es
              in  (es', earr', u', tc')
            ...
            h0  = countComps uf' n
            nE  = length curEdges'
            -- Euler relation: χ = V - E + T, H1 = E - V + H0 - T
